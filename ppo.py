...
class PPO(Agent):
    ...
    def act(self, states):
        ...
        self._train(states)
        self._states = states
        self._actions = self.policy.no_grad(states).sample()
        return self._actions

    def eval(self, states):
        return self.policy.eval(states)
    ...
    def _train(self, next_states):
        if len(self._buffer) >= self._batch_size:
            ...
            pi_0 = self.policy.no_grad(states).log_prob(actions)
            targets = self.v.no_grad(states) + advantages
